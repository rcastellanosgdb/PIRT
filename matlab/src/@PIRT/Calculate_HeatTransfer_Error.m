...
if obj.HeatTransfer_params.compute_St
    ...
    if ~any(strcmp(err,'errorcpinf'))
        error(['PIRT:parse_PIRT_HeatTransfer: The error in the cpinf ' ...
            'value must be introduced to calculate the heat transfer ' ...
            'error estimation for St'])
    end
    Uinf        = obj.HeatTransfer_params.conditions.Uinf;
    ...
    rhoinf      = obj.HeatTransfer_params.conditions.rhoinf;
    errorrhoinf = obj.HeatTransfer_params.Error.errorrhoinf;
    cpinf       = obj.HeatTransfer_params.conditions.cpinf;
    errorcpinf  = obj.HeatTransfer_params.Error.errorcpinf;
end

% Film temperature defined as in Calculate_HeatTransfer
...
for i=1:n
    ...
    %-- Radiative heat flux: qr = sides·σ·ε·(Th⁴-Tc⁴)  (same model as Calculate_HeatTransfer)
    q    = q - sides*sigma.*current.epsilon.*(current.Thot.^4-current.Tamb_hot.^4); % [W/m^2]
    ...
    if obj.HeatTransfer_params.compute_St
        ...
        current.cpinf   = normrnd(cpinf,abs(cpinf.*errorcpinf));
        St(:,:,i) = h(:,:,i)./(current.rhoinf*current.cpinf*current.Uinf); % fluid properties
    end
end
