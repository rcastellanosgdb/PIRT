...
if any(strcmp(params,'legacy_transform'))
    idx = find(strcmp(params,'legacy_transform'));
    mPOD_params.legacy_transform = params{idx+1};
    if ~islogical(mPOD_params.legacy_transform) && ~isnumeric(mPOD_params.legacy_transform)
        error('PIRT:parse_PIRT_filter:legacy_transform for the mPOD filter must be true or false')
    end
    mPOD_params.legacy_transform = logical(mPOD_params.legacy_transform);
end
