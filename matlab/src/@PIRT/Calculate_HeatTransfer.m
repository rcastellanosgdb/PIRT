...
%           k: thermal conductance of the foil, thickness x conductivity [W/K]
...
if obj.HeatTransfer_params.compute_St
    % St = h/(rho_inf cp_inf U_inf) with the properties of the FLUID.
    % (v1.1: previous versions used the specific heat of the foil, HFS.cp)
...
if params.compute_St
    ...
    if ~any(strcmp(condition_data,'cpinf'))
        error('PIRT:Calculate_HeatTransfer: cpinf (fluid specific heat) must be introduced to compute the heat transfer if the St number is desired')
    end
end
