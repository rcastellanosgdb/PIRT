...
if any(strcmp(varargin,'Conditions'))
    ...
    if ~isempty(cond)
        ...
        if any(strcmp(cond,'cpinf'))
            idx = find(strcmp(cond,'cpinf'));
            conditions.cpinf = cond{idx+1};
            if ~isfloat(conditions.cpinf)
                error('PIRT:parse_PIRT_HeatTransfer:The Conditions free stream specific heat "cpinf" must be introduced as a float value')
            end
        else
            warning('PIRT:parse_PIRT_HeatTransfer:The Conditions free stream specific heat must be intoduced to calculate the heat transfer if the St number is desired, it should be identified as "cpinf"')
        end
...
if any(strcmp(varargin,'Error'))
    ...
    if ~isempty(err)
        ...
        if any(strcmp(err,'errorcpinf'))
            idx = find(strcmp(err,'errorcpinf'));
            Error.errorcpinf = err{idx+1};
            if ~isfloat(Error.errorcpinf)
                error('PIRT:parse_PIRT_HeatTransfer: The error in the cpinf must be introduced as a float value')
            end
        end
