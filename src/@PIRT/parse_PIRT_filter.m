...
if ~or(ischar(filters),isstring(filters))
    ...
    for i=1:n_filters
        [pod,golay,mpod,wiener,filter_type(i)] = select_filter(filters{i},params{i});
        POD_params{i} = pod;
    ...
    [pod,golay,mpod,wiener,filter_type] = select_filter(filters,params);
    POD_params = cell(1,1);
...
function [POD_params,sgolay32_params,mPOD_params,wiener3_params,filter_type] = select_filter(filter,params)
